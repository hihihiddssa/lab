...
class DobotControl:
    def __init__(self):
        ...
        # 添加夹爪状态变量
        self.gripper_state = {
            'left': False,  # False表示关闭，True表示打开
            'right': False
        }
        
        # 初始化DO输出
        self.setup_do_outputs()
    ...
    def setup_ui(self):
        ...
        ttk.Label(left_frame, text="左夹爪").pack()
        ttk.Button(left_frame, text="打开", 
                  command=lambda: self.control_gripper("left", True)).pack(side="left", padx=5)
        ttk.Button(left_frame, text="关闭", 
                  command=lambda: self.control_gripper("left", False)).pack(side="left", padx=5)
        
        # 右夹爪控制
        right_frame = ttk.Frame(gripper_frame)
        right_frame.pack(side="left", padx=10, pady=5)
        ttk.Label(right_frame, text="右夹爪").pack()
        ttk.Button(right_frame, text="打开", 
                  command=lambda: self.control_gripper("right", True)).pack(side="left", padx=5)
        ttk.Button(right_frame, text="关闭", 
                  command=lambda: self.control_gripper("right", False)).pack(side="left", padx=5)
    ...
    def initialize_robot(self):
        try:
            ...
            robot_state = self.dashboard.RobotMode()
            if "0" not in robot_state:  # 检查是否仍有错误
                raise Exception(f"清除错误失败,当前状态: {robot_state}")
    ...
    def __del__(self):
        ...
        try:
            # 关闭所有DO输出
            self.dashboard.SetDO(1, 0)
            self.dashboard.SetDO(2, 0)
            self.dashboard.SetDO(3, 0)
    ...
    def control_gripper(self, which_gripper, open_state):
        """
        控制夹爪的开合
        which_gripper: "left" 或 "right"，指定控制哪个夹爪
        open_state: True表示打开，False表示关闭
        """
        try:
            if which_gripper == "left":
                do_port = 1  # 左夹爪使用DO1
            else:
                do_port = 2  # 右夹爪使用DO2
            
            # 设置DO输出
            state = 1 if open_state else 0
            self.dashboard.SetDO(do_port, state)
            
            # 更新状态
            self.gripper_state[which_gripper] = open_state
            
            print(f"{which_gripper}夹爪已{'打开' if open_state else '关闭'}")
            
        except Exception as e:
            print(f"夹爪控制错误: {str(e)}")

    def setup_do_outputs(self):
        """初始化数字输出状态"""
        try:
            # 初始化所有相关的DO为关闭状态
            self.dashboard.SetDO(1, 0)  # DO1 - 左夹爪
            self.dashboard.SetDO(2, 0)  # DO2 - 右夹爪
            self.dashboard.SetDO(3, 0)  # DO3 - 备用
            print("数字输出初始化完成")
        except Exception as e:
            print(f"初始化数字输出错误: {str(e)}")
