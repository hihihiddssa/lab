...
image_left, image_right, image_bottom, thread_run = None, None, None, None
lock = threading.Lock()


# 定义摄像头线程函数，根据摄像头索引读取图像数据
def run_thread_cam(rs_cam, which_cam):
    global image_left, image_right, image_bottom, thread_run
    if which_cam == 0:
        while thread_run:
            # 读取图像并转换颜色通道顺序
...
def main(args):
    ...
    rs3 = RealSenseCamera(flip=True, device_id=camera_dict["bottom"])
    ...
    thread_cam_bottom = threading.Thread(target=run_thread_cam, args=(rs3, 2))
    # 启动线程
    ...
    observation = {'qpos': [], 'images': {'left_wrist': [], 'right_wrist': [], 'bottom': []}}
    obs = env.get_obs()
    # 设置夹爪的初始位置
    ...
    while t < episode_len:
        ...
        observation['images']['bottom'] = image_bottom
        if args.show_img:
            # 拼接图像并显示
        ...
        if not ((pos[0] > -410 and pos[0] < 210 and pos[1] > -700 and pos[1] < -210 and pos[2] > 22) and \
                (pos[6] < 410 and pos[6] > -210 and pos[7] > -700 and pos[7] < -210 and pos[8] > 22)):
